...
parser.add_argument('strategy',type=int,help='Strategy')
parser.add_argument('wf', type=float, help='wf')
...
bounds = FloatVar(lb=(lb,) * int(n_vars), ub=(ub,) * int(n_vars), name=name)
problem_dict = {
    "bounds": bounds,
    "minmax": minmax,
...
model = DE.OriginalDE(epoch=epoch, pop_size=popSize, wf = wf, cr = cr, strategy = strategy)
g_best = model.solve(problem_dict)